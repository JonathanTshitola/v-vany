...
export default function Cart({ cart, setCart, session, setView }) {
  ...
  // --- Étape 1 : Enregistrement avec récupération des infos Profil ---
  const handleFinalizeOrder = async () => {
    if (!session) { setView('login'); return; }
    setIsProcessing(true);

    try {
      // 1. Récupérer les infos du profil pour l'Admin
      ...
      // 2. Enregistrement de la commande complète
  ...
  if (ordered) return (
    <div className="max-w-2xl mx-auto px-4 py-10 animate-fadeIn">
      ...
      <div className="bg-zinc-900 border border-zinc-700 p-8 rounded-[2.5rem] shadow-2xl text-center space-y-8">
        <div className="py-4">
        ...
        <div className="border-t border-zinc-800 pt-8 space-y-4">
          ...
          >
            Notifer le service client (WhatsApp)
          </button>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 backdrop-blur-md bg-black/80 animate-fadeIn">
          <div className="bg-zinc-900 border border-zinc-700 w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl text-center">
            ...
            <p className="text-zinc-400 text-[11px] mb-8 leading-relaxed">
              Prêt à envoyer votre preuve de paiement sur WhatsApp ?
            </p>
            <div className="flex flex-col gap-3">
              <button onClick={redirectToWhatsApp} className="w-full bg-[#25D366] text-white py-4 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-lg">Continuer sur WhatsApp</button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-8 rounded-[2.5rem] sticky top-32 text-black shadow-2xl">
              <h3 className="font-black uppercase text-[10px] tracking-widest mb-8 border-b border-black/10 pb-4 text-center">Sommaire</h3>
              ...
              >
                {isProcessing ? "RÉSERVATION..." : "CONFIRMER LA COMMANDE"}
              </button>
            </div>
          </div>
        </div>
